...
function ContractsPage() {
  ...
  const { data: contracts = [], isLoading, error } = useQuery({
    ...
    queryFn: () => projectId ? contractService.getContractsByProjectId(projectId) : contractService.getAllContracts(),
    staleTime: 30_000,
  });
  ...
  const filtered = contracts.filter((c: Contract) => {
    ...
    return (
      (!q || c.contractNumber.toLowerCase().includes(q) || c.clientName.toLowerCase().includes(q)) &&
      (statusFilter === 'all' || c.status.toLowerCase().includes(statusFilter.toLowerCase()))
    );
  });

  // Metrics
  const totalValue = contracts.reduce((s, c) => s + (Number(c.revisedContractValue) || 0), 0);
  const totalVariations = contracts.reduce((s, c) => s + (Number(c.totalApprovedVariations) || 0), 0);
  const activeCount = contracts.filter(c => ['active', 'Active', 'ACTIVE'].includes(c.status)).length;
  ...
  // Map contract → initialData for the form
  const contractToFormData = (c: Contract) => ({
    projectId: c.projectId,
    contractNumber: c.contractNumber,
    contractType: c.contractType,
    clientName: c.clientName,
    originalContractValue: Number(c.originalContractValue) || 0,
    originalDurationDays: Number(c.originalDurationDays) || 0,
    startDate: c.startDate ? c.startDate.split('T')[0] : '',
    completionDate: c.completionDate ? c.completionDate.split('T')[0] : '',
    retentionPercentage: Number(c.retentionPercentage) || 0,
    advancePaymentAmount: Number(c.advancePaymentAmount) || 0,
    performanceBondValue: Number(c.performanceBondValue) || 0,
  ...
  return (
    <div className="space-y-5 pb-8 min-w-0">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <InteractiveCard icon={CheckCircle} iconColor="text-green-400"  title="Active"           value={activeCount} badge={{ text: 'Live', color: 'text-green-400' }} />
      ...
      {error && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-5">
          ...
          <button onClick={() => queryClient.invalidateQueries({ queryKey: ['contracts'] })}
            className="mt-2 px-4 py-1.5 bg-red-600 text-white rounded-lg text-sm">Retry</button>
        </div>
      )}
      ...
      <div className="bg-[#0A0A0A] border border-gray-800 rounded-xl overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center">
            ...
            <h3 className="font-semibold text-white mb-2">{contracts.length === 0 ? 'No contracts yet' : 'No contracts match'}</h3>
            <p className="text-gray-500 text-sm mb-5">{contracts.length === 0 ? 'Create the first contract for this project.' : 'Try adjusting your search or status filter.'}</p>
            {contracts.length === 0 && (
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-gray-800">
                  {['Contract #', 'Client', 'Type', 'Value', 'Status', 'Start', ''].map(h => (
              ...
              <tbody className="divide-y divide-gray-800/50">
                {filtered.map(c => (
                  <tr key={c.id} className="hover:bg-[#111] transition-colors group cursor-pointer"
                    ...
                    <td className="px-5 py-4 text-xs text-gray-400">{c.contractType || '—'}</td>
                    <td className="px-5 py-4 text-right">
                      <p className="text-sm font-semibold text-white font-mono">
                        {Number(c.revisedContractValue || c.originalContractValue || 0).toLocaleString('en-KE')}
                      </p>
                      {Number(c.totalApprovedVariations) > 0 && (
                        <p className="text-[10px] text-yellow-400">+{Number(c.totalApprovedVariations).toLocaleString()} var.</p>
                      )}
                    </td>
                    <td className="px-5 py-4">
                      <span className={`px-2.5 py-0.5 text-xs font-semibold rounded-lg border ${statusColor(c.status || 'Draft')}`}>
      ...
      {/* Modal — uses initialData (not contract prop) */}
      {isModalOpen && (
        <ContractFormModal
          projectId={projectId}
          initialData={editingContract ? contractToFormData(editingContract) : undefined}
          isOpen={isModalOpen}
